...
from .config import MODEL_NAME, MODEL_CONTENT_PREPROC, TRAIN_COLUMNS, CATEGORY_MAPPINGS
...
with open(settings.ENCODED_CATEGORY_PATH, 'r') as json_file:
    category_mean_dict = json.load(json_file)
...
async def create_process(user_id: int, dataset: PredictItem):
    ...
    features = serpg.ready_features(data=data,
                                    encoder=encoder, 
                                    model=model,
    ...
    # message = {'user_id': user_id, 'data': features}
